...
public class AdvancedTextAnalyzer {
    ...
    private void initializePatterns() {
        emailPattern = Pattern.compile("\\b[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\\.[A-Z|a-z]{2,}\\b");
    ...
    private void analyzeStructuralElements(String text, AdvancedTextStatistics stats) {
    ...
    private void analyzeLanguageFeatures(String text, AdvancedTextStatistics stats) {
        ...
        stats.setAllCapsWord(allCapsWords);
    ...
    private void analyzeSentenceComplexity(String text, AdvancedTextStatistics stats) {
        ...
        stats.setAverageWordsPerSentence(avgWordsPerSentence);
    }
    ...
    private void analyzeMarkdownFeatures(String text, Map<String, Double> scores) {
        ...
        // Detect markdown features such as headers, lists, code blocks, links, images
        int headerCount = countPattern(text, "^#");
        int listCount = countPattern(text, "^[-*+] ");
        int codeBlockCount = countPattern(text, "^(?:```|~~~)");
        int linkCount = countPattern(text, "\\[.*?\\]\\(.*?\\)");
        int imageCount = countPattern(text, "!\\[.*?\\]\\(.*?\\)");
        scores.put("markdown_headers", (double) headerCount);
        scores.put("markdown_lists", (double) listCount);
        scores.put("markdown_code_blocks", (double) codeBlockCount);
        scores.put("markdown_links", (double) linkCount);
        scores.put("markdown_images", (double) imageCount);
    }
}